...
class apache_gen(log_gen):
    ...
    def methods_p(self, val):
        ...
        if len(val) != len(self.methods):
            raise Exception(
                "Length of methods_p doesn't equal length of methods.")
        if abs(1-sum(val)) > 0.01:
            raise Exception("Sum of methods_p must equals 1.")
        for x in val:
            if x < 0 or x > 1:
                raise Exception(
                    "All members of methods_p must be in the range of 0 to 1 ")
        self._methods_p = val
    ...
    def heartbeat_interval(self, val):
        if type(val) != type(1) and type(val) != type(0.5):
            raise Exception(
                "heartbeat_interval value should be either integer or decimal")
        if val is not None and 'heartbeat' not in self.lines:
            raise Exception(
                "Only set heartbeat_interval when generate heartbeat")
    ...
    def access_interval(self, val):
        ...
        if len(val) != 2:
            raise Exception(
                "access_interval should be a list containing 2 elements")
        if type(val[0]) != type(1) and type(val[0]) != type(0.5):
            raise Exception(
                "access_interval[0] should be either integer or decimal")
        if type(val[1]) != type(1) and type(val[1]) != type(0.5):
            raise Exception(
                "access_interval[1] should be either integer or decimal")
        if val[0] >= val[1]:
            raise Exception(
                "access_interval[0] should be smaller than access_interval[1]")
    ...
    def run(self):
        ...
        try:
            loop.run_until_complete(
                asyncio.wait([self.lines_gen[self.lines_full.index(x)]
                              for x in self.lines])
            )
        finally:
            loop.close()
    ...
    def access_lines(self):
        while self.forever or self.count > 0:
            ...
            self.output_access(ip, user_identifier,
                               user_id, t, msg, code, size)
    ...
    def output_access(self, ip, user_identifier, user_id, t, msg, code, size):
        string = ip+' '+user_identifier+' '+user_id + \
            ' '+'['+t+'] "'+msg+'" '+code+' '+str(size)
        if 'stdout' in self.out_format:
            print(string)
        if 'log' in self.out_format:
