...
from esloggerhandler.serializers import ESLoggerSerializer


class ESLoggerHandler(logging.Handler):
    ...
    def __init__(self,
        ...
        :param auth_details: When ```ESLoggerHandler.AuthType.BASIC_AUTH``` is used this argument must contain
        ...
        :param auth_type: The authentication type to be used in the connection ```ESLoggerHandler.AuthType```
        ...
        :param raise_on_indexing_exceptions: A boolean, True only for debugging purposes to raise exceptions
                    caused when
        :return: A ready to be used ESLoggerHandler.
        """
        ...
        self._timer = None
        self._index_name_func = ESLoggerHandler._INDEX_FREQUENCY_FUNCION_DICT[self.index_name_frequency]
        self.serializer = ESLoggerSerializer()
    ...
    def emit(self, record):
        ...
        for key, value in record.__dict__.items():
            if key not in ESLoggerHandler.__LOGGING_FILTER_FIELDS:
                if key == "args":
